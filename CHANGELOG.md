...
- Math (`$...$` inline and `$$...$$` block) now renders with the same KaTeX plugin that VS Code uses for its built-in Markdown preview, keeping math behavior consistent across the editor
...
- Fix D2 diagrams becoming permanently unrenderable after a memory error; the renderer now recovers automatically without requiring a VS Code restart

### Changed

- Cache D2 diagram renders so that previously rendered diagrams display instantly

## 0.7.2 - 2026-04-24

### Fixed

- Fix D2 diagrams failing to render on Windows with `Only URLs with a scheme in: file, data, node, and electron are supported` error
