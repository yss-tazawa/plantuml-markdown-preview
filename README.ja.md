<p align="center">
  <a href="https://yss-tazawa.github.io/plantuml-markdown-preview/">English</a> | <a href="https://yss-tazawa.github.io/plantuml-markdown-preview/#/zh-cn/">简体中文</a> | <strong>日本語</strong> | <a href="https://yss-tazawa.github.io/plantuml-markdown-preview/#/es/">Español</a> | <a href="https://yss-tazawa.github.io/plantuml-markdown-preview/#/pt-br/">Português</a> | <a href="https://yss-tazawa.github.io/plantuml-markdown-preview/#/ko/">한국어</a> | <a href="https://yss-tazawa.github.io/plantuml-markdown-preview/#/zh-tw/">繁體中文</a>
</p>
...
<p align="center">
  <img src="images/hero-screenshot.png" width="800" alt="GitHub Light テーマでシーケンス図を表示しているエディタとプレビューの並び">
</p>
...
- **スタンドアロンダイアグラムプレビュー** — `.puml`、`.plantuml`、`.mmd`、`.mermaid`、`.d2` ファイルをパン＆ズーム、ライブ更新、テーマ対応で直接プレビュー — Markdown ラッパー不要
...
- **D2 スケール** — `auto`（コンテナ幅に合わせる）または固定パーセンテージ（50%〜100%、デフォルト 70%）。
...
- **Fast モード**（デフォルト） — `localhost` で常駐 PlantUML サーバーを実行。初回のダイアグラムレンダリング時に遅延起動します（設定変更可 — [起動モード](#fast-モード-サーバーの起動モード)を参照）。編集ごとの JVM 起動コストを排除し、即座に再レンダリング。ダイアグラムはマシン外に送信されません。
...
デフォルトでは、Fast モードは `127.0.0.1` にバインドされた PlantUML サーバーを自動的に起動・管理します — 設定は不要です。サーバーをいつ起動するか（そもそも起動するかどうか）は `plantumlLocalServerStartMode` で制御します:

| 値 | 効果 |
|---|---|
| `"lazy"`（デフォルト） | 初回のダイアグラムレンダリング時に管理下のサーバーを起動します。ダイアグラムを一切レンダリングしない間は JVM が動作しません。最初のレンダリングだけ数秒余分にかかります。 |
| `"on"` | 拡張機能のアクティベート時に管理下のサーバーを起動します。最初のレンダリングは即座に行われますが、起動直後から JVM が常駐するコストがかかります。 |
| `"off"` | サーバーを起動しません。自分で起動したサーバー（例: `java -jar plantuml.jar -picoweb`）に接続します。LAN 上の別のマシンで動作しているサーバーも指定できます: |

| 設定 | デフォルト | 説明 |
|---|---|---|
| `plantumlLocalServerHost` | `127.0.0.1` | Start Mode が `"off"` のときに接続するホスト（例: LAN 上の別の場所にある picoweb サーバー）。`"on"`/`"lazy"` のときは無視されます — 管理下のサーバーは常に `127.0.0.1` にバインドされます。 |
| `plantumlLocalServerPort` | `0` | Start Mode `"on"`/`"lazy"`: 起動するポート（`0` は空いているポートを自動割り当て）。Start Mode `"off"`: 接続先のポート。 |

補足:

- 管理モード（`"on"`/`"lazy"`）で固定ポートを指定している場合、そのポートで正常な PlantUML サーバーが既に動作していれば、拡張機能は新たに起動せずそれを再利用します — そのため、残存プロセスや自分で起動したサーバーと競合しなくなります。
- Start Mode が `"off"` の場合、拡張機能はサーバーを起動しません（ローカルの Java も不要です）— 設定したホスト・ポートに接続するだけです。
- 拡張機能が停止するのは、拡張機能自身が起動したサーバーだけです。ユーザーが起動したサーバーを拡張機能が終了することはありません。
- 旧設定の `plantumlLocalServerAutoStart`（真偽値）は非推奨ですが、Start Mode が明示的に設定されていない場合は引き続き有効です（`true` → `"on"`、`false` → `"off"`）。明示的に設定された Start Mode は常に優先されます。

#### Fast モード: JVM メモリ（ヒープ）設定

管理下のサーバーのメモリ使用量は、ヒーププリセット（`plantumlLocalServerJvmHeapPreset`）で上限を設定します:

| プリセット | JVM ヒープフラグ | 使いどころ |
|---|---|---|
| `small` | `-Xms16m -Xmx256m` | メモリ使用量最小。小規模なダイアグラムのみ向け。 |
| `medium`（デフォルト） | `-Xms16m -Xmx512m` | バランス型。ほとんどのダイアグラムを問題なく処理。 |
| `large` | `-Xms64m -Xmx1024m` | 非常に大きなダイアグラム向け。PlantUML FAQ の推奨値と一致。 |
| `unlimited` | _（なし）_ | フラグを一切付けず JVM に任せる（最大ヒープはデフォルトで物理 RAM の 1/4）。v0.7.10 より前の挙動へのフォールバック。 |
| `custom` | 任意の値 | `plantumlLocalServerJvmInitialHeapMb`（8〜32768、デフォルト 16）と `plantumlLocalServerJvmMaxHeapMb`（64〜32768、デフォルト 512）を使用。 |

`unlimited` を除く全プリセットでは、サーバーを小さく保つために `-XX:+UseSerialGC`、`-XX:MaxMetaspaceSize=128m`、`-XX:ReservedCodeCacheSize=64m` も固定で付与されます。これらの設定は、拡張機能自身が起動するサーバー（Fast モード、Start Mode `"on"`/`"lazy"`）にのみ適用されます。

### ステータスバー

ステータスバーに現在のレンダリングモード（Fast / Secure / Easy）と、Fast モード時はローカルサーバーの状態（起動中・稼働中・エラー・停止中）を表示します。

- ステータスバー項目をクリックすると、設定を開かずにクイックピックでモードを切り替え可能
- コマンドパレットの **レンダリングモードを選択** と同じ操作

### HTML エクスポート

Markdown ドキュメントを自己完結型の HTML ファイルにエクスポートできます。

- PlantUML・Mermaid・D2 ダイアグラムをインライン SVG として埋め込み
- シンタックスハイライト CSS 込み — 外部依存なし
- エクスポートしてブラウザで開くコマンドをワンステップで
- レイアウト幅（640px〜1440px または無制限）と配置（中央寄せ / 左寄せ）を設定可能
- **ピクセル幅プリセット**（640〜1440px、中央寄せ）と、ビューポート幅に追従する **レスポンシブ** モード（左寄せ）

### PDF エクスポート

ヘッドレス Chromium ベースのブラウザを使用して Markdown ドキュメントを PDF にエクスポートできます。

- Chrome、Edge、または Chromium がシステムにインストールされている必要があります
- 縦向き・横向きに対応
- テキストとダイアグラムを均等にスケール（`pdfScale` 設定、デフォルト 0.625）
- 印刷マージンが適用され、きれいなレイアウトに

### 一括ダイアグラムエクスポート

Markdown ファイル内のすべてのダイアグラムを個別の PNG または SVG ファイルとしてエクスポート。

- すべての PlantUML / Mermaid / D2 ダイアグラムをドキュメント順にエクスポート
- 出力ディレクトリ名はカスタマイズ可能（デフォルト: `{ファイル名}_diagrams`）
- プレビューが開いている必要あり（Webview 経由でレンダリング）
...
> **注意:** `plantumlIncludePath`（およびワークスペースルート基準の解決）が効くのは、拡張機能自身が PlantUML プロセスを起動した場合のみです — Secure モードでは常に有効ですが、Fast モードでは拡張機能が自分でローカルサーバーを起動した場合に限られます。Fast モードで既に起動しているサーバーに接続した場合（Start Mode `"off"`、または固定ポートで既存サーバーを再利用した場合）は、そのサーバー自身の作業ディレクトリを基準にインクルードが解決されます。Fast モードで確実に設定を効かせるには、ポートを `0`（自動）に設定してください。
...
| ツール | 用途 | 確認方法 |
| ------ | ---- | -------- |
| [Java 11+（JRE または JDK）](#セットアップ) | PlantUML を実行（バンドル済み PlantUML 1.2026.2 は Java 11+ が必要） | `java -version` |
...
**Fast モード**（デフォルト）：常駐ローカル PlantUML サーバーを実行して即座に再レンダリング。初回のダイアグラムレンダリング時に起動します。Java 11+ が必要。
...
> **注意：** `javaPath` のデフォルトは `"java"`。デフォルトのままの場合、`JAVA_HOME/bin/`（Windows では `JAVA_HOME\bin\`）配下の Java バイナリを最初に試し、次に PATH 上の `java` を使用。`dotPath` と `plantumlJarPath` のデフォルトはそれぞれ `"dot"` と同梱 jar。これらのコマンドが PATH にない場合や別の jar を使いたい場合のみ設定する。
...
- **コンテキストメニュー：** `.md` ファイルを右クリック → **PlantUML Markdown Preview** → **HTML としてエクスポート** → 幅オプションを選択
- **プレビューパネル：** プレビュー内を右クリック → **HTML としてエクスポート** または **HTML としてエクスポートして開く**

サブメニューで幅オプションを選択できます：

| オプション | 説明 |
| ------ | ----------- |
| Current Settings | 現在の設定でエクスポート（レスポンシブ CSS なし） |
| 640px – 1440px | ピクセル単位の固定ボディ幅、中央寄せ |
| Responsive (Left-Aligned) | ボディをビューポート幅一杯に広げ左寄せ。はみ出したダイアグラムのみ縮小 |
| 640px – 1440px (Left-Aligned) | 固定幅、左寄せ |

HTML ファイルはソースの `.md` ファイルと同じ場所に保存される。

### PDF にエクスポート

- **コンテキストメニュー：** `.md` ファイルを右クリック → **PlantUML Markdown Preview** → **PDF としてエクスポート** → **縦向き** または **横向き**
- **プレビューパネル：** プレビュー内を右クリック → **PDF としてエクスポート** または **PDF としてエクスポートして開く** → **縦向き** または **横向き**

PDF ファイルはソースの `.md` ファイルと同じ場所に保存される。Chrome、Edge、または Chromium が必要。`pdfScale` 設定（デフォルト 0.625）でテキストとダイアグラムの全体的なスケールを調整できます。
...
- **コンテキストメニュー：** `.md` ファイルを右クリック → **PlantUML Markdown Preview** → **Export All Diagrams as PNG** または **Export All Diagrams as SVG**
- **プレビューパネル：** プレビュー内を右クリック → **Export All Diagrams as PNG** または **Export All Diagrams as SVG**
- **コマンドパレット：** `PlantUML Markdown Preview: Export All Diagrams as PNG` / `Export All Diagrams as SVG`

保存ダイアログで出力ディレクトリ名を選択（デフォルト: `{ファイル名}_diagrams`）。各ダイアグラムは `diagram-1.png`、`diagram-2.png` のようにドキュメント順で保存。プレビューが開いている必要あり。
...
| `previewTheme` | `"github-light"` | プレビューテーマ（[テーマ](#テーマ)を参照） |
...
| `d2Scale` | `"70%"` | D2 ダイアグラムスケール。`"auto"` はコンテナ幅に合わせてスケール。パーセンテージ（50%〜100%）は自然サイズのその割合でレンダリング。 |
...
| `pdfScale` | `0.625` | PDF エクスポートのスケール。テキストとダイアグラムを均等に拡縮。範囲: 0.1〜2.0。 |
...
| `plantumlLocalServerStartMode` | `"lazy"` | ローカル PlantUML サーバーの起動タイミング（Fast モードのみ）：`"on"`（アクティベート時）、`"lazy"`（初回のダイアグラムレンダリング時）、`"off"`（起動しない — 既存のサーバーに接続）。[起動モード](#fast-モード-サーバーの起動モード)を参照。 |
| `plantumlLocalServerHost` | `"127.0.0.1"` | Start Mode が `"off"` のときに接続するホスト（Fast モードのみ）。`"on"`/`"lazy"` のときは無視されます。 |
| `plantumlLocalServerPort` | `0` | ローカル PlantUML サーバーのポート（Fast モードのみ）。`0` = 空きポートを自動割り当て。Start Mode `"off"` の場合は接続先のポート。 |
| `plantumlLocalServerJvmHeapPreset` | `"medium"` | 管理下のローカルサーバーの JVM ヒーププリセット：`"small"`、`"medium"`、`"large"`、`"unlimited"`、`"custom"`。[JVM メモリ設定](#fast-モード-jvm-メモリヒープ設定)を参照。 |
| `plantumlLocalServerJvmInitialHeapMb` | `16` | `"custom"` プリセット用の初期ヒープ（`-Xms`、MB）。他のプリセットでは無視されます。範囲: 8〜32768。 |
| `plantumlLocalServerJvmMaxHeapMb` | `512` | `"custom"` プリセット用の最大ヒープ（`-Xmx`、MB）。他のプリセットでは無視されます。範囲: 64〜32768。 |
...
- Fast モードでは、インクルードの基準ディレクトリは拡張機能が自分でローカルサーバーを起動した場合のみ有効です。既に起動しているサーバーに接続する場合（Start Mode `"off"`、または固定ポートでの既存サーバー再利用）は、そのサーバーの作業ディレクトリが基準になります — ポートを `0`（自動）に設定すると、拡張機能が必ず自分でサーバーを起動するようになります。
