...
- **Fast モード**（デフォルト） — `localhost` で常駐 PlantUML サーバーを起動。編集ごとの JVM 起動コストを排除し、即座に再レンダリング。ダイアグラムはマシン外に送信されません。
...
#### Fast モード: 独自の PlantUML サーバーに接続する

デフォルトでは、Fast モードは `127.0.0.1` にバインドされた PlantUML サーバーを自動的に起動・管理します — 設定は不要です。自分で起動したサーバー（例: `java -jar plantuml.jar -picoweb`）を指定することもでき、LAN 上の別のマシンで動作しているサーバーを指定することもできます。

| 設定 | デフォルト | 説明 |
|---|---|---|
| `plantumlLocalServerAutoStart` | `true` | オン: 拡張機能がサーバーを起動・管理します。オフ: サーバーを起動せず、既存のサーバーに接続します。 |
| `plantumlLocalServerHost` | `127.0.0.1` | 自動起動が**オフ**のときに接続するホスト（例: LAN 上の別の場所にある picoweb サーバー）。自動起動がオンのときは無視されます — 管理下のサーバーは常に `127.0.0.1` にバインドされます。 |
| `plantumlLocalServerPort` | `0` | 自動起動が**オン**の場合: 起動するポート（`0` は空いているポートを自動割り当て）。自動起動が**オフ**の場合: 接続先のポート。 |

補足:

- 自動起動が**オン**で固定ポートを指定している場合、そのポートで正常な PlantUML サーバーが既に動作していれば、拡張機能は新たに起動せずそれを再利用します — そのため、残存プロセスや自分で起動したサーバーと競合しなくなります。
- 自動起動が**オフ**の場合、拡張機能はサーバーを起動しません（ローカルの Java も不要です）— 設定したホスト・ポートに接続するだけです。
- 拡張機能は自分で起動したサーバーのみを停止します。自分で起動したサーバーが拡張機能によって終了されることはありません。
