<p align="center">
  <a href="https://yss-tazawa.github.io/plantuml-markdown-preview/">English</a> | <strong>简体中文</strong> | <a href="https://yss-tazawa.github.io/plantuml-markdown-preview/#/ja/">日本語</a> | <a href="https://yss-tazawa.github.io/plantuml-markdown-preview/#/es/">Español</a> | <a href="https://yss-tazawa.github.io/plantuml-markdown-preview/#/pt-br/">Português</a> | <a href="https://yss-tazawa.github.io/plantuml-markdown-preview/#/ko/">한국어</a> | <a href="https://yss-tazawa.github.io/plantuml-markdown-preview/#/zh-tw/">繁體中文</a>
</p>
...
<p align="center">
  <img src="images/hero-screenshot.png" width="800" alt="编辑器与预览并排显示，GitHub Light 主题下的序列图">
</p>
...
- **独立图表预览** — 直接预览 `.puml`、`.plantuml`、`.mmd`、`.mermaid`、`.d2` 文件，支持平移缩放、实时更新和主题 — 无需 Markdown 包装
...
- **像素宽度预设**（640–1440px，居中对齐）和扩展到视口宽度的 **响应式** 模式（左对齐）
...
将 Markdown 文件中的所有图表导出为单独的 PNG 或 SVG 文件。

- 按文档顺序导出所有 PlantUML / Mermaid / D2 图表
- 输出目录名称可自定义（默认：`{文件名}_diagrams`）
- 需要打开预览（通过预览 Webview 渲染）
...
- **打开包含源文件** — 在预览中右键点击 PlantUML 图表，直接打开其包含文件
- 适用于 Fast 和 Secure 模式。Easy 模式不可用（远程服务器无法访问本地文件）

> **注意：** `plantumlIncludePath`（以及基于工作区根目录的解析）仅在扩展自身启动 PlantUML 进程时才生效 — 在 Secure 模式下始终生效，但在 Fast 模式下仅当扩展启动了自己的本地服务器时才生效。如果 Fast 模式连接到一个已在运行的服务器（Start Mode 为 `"off"`，或在固定端口上复用现有服务器），包含文件则会改为相对于该服务器自身的工作目录解析。要确保该设置在 Fast 模式下生效，请将端口设置为 `0`（自动）。
...
| 工具 | 用途 | 验证 |
| --- | --- | --- |
| [Java 11+（JRE 或 JDK）](#设置) | 运行 PlantUML（内置 PlantUML 1.2026.2 需要 Java 11+） | `java -version` |
...
| 640px – 1440px | 固定像素宽度，居中 |
| Responsive (Left-Aligned) | 正文宽度等于视口，靠左对齐；溢出的图表才会缩小 |
...
- **上下文菜单：** 右键点击 `.md` 文件 → **PlantUML Markdown Preview** → **Export All Diagrams as PNG** 或 **Export All Diagrams as SVG**
- **预览面板：** 在预览面板内右键 → **Export All Diagrams as PNG** 或 **Export All Diagrams as SVG**
- **命令面板：** `PlantUML Markdown Preview: Export All Diagrams as PNG` / `Export All Diagrams as SVG`

保存对话框可选择输出目录名称（默认：`{文件名}_diagrams`）。每个图表按文档顺序保存为 `diagram-1.png`、`diagram-2.png` 等。需要打开预览。
...
- 在 Fast 模式下，`!include` 指令的基础目录仅在扩展启动了自己的本地服务器时才生效。如果你连接到一个已在运行的服务器（Start Mode 为 `"off"`，或在固定端口上复用现有服务器），包含文件则会改为相对于该服务器的工作目录解析 — 将端口设置为 `0`（自动），可让扩展始终启动自己的服务器。
