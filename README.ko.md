...
- **Fast 모드** (기본값) — `localhost`에서 상주 PlantUML 서버를 시작합니다. 편집할 때마다 발생하는 JVM 시작 비용을 제거하여 즉각적인 재렌더링이 가능합니다. 다이어그램은 사용자의 컴퓨터를 벗어나지 않습니다.
...
#### Fast 모드: 자체 PlantUML 서버에 연결하기

기본적으로 Fast 모드는 `127.0.0.1`에 바인딩된 PlantUML 서버를 직접 시작하고 관리합니다 — 별도의 설정이 필요 없습니다. 직접 실행하는 서버(예: `java -jar plantuml.jar -picoweb`)를 가리키도록 설정할 수도 있으며, LAN 내 다른 컴퓨터에서 실행 중인 서버도 지정할 수 있습니다.

| 설정 | 기본값 | 효과 |
|---|---|---|
| `plantumlLocalServerAutoStart` | `true` | 켜짐: 확장 프로그램이 서버를 시작하고 관리합니다. 꺼짐: 서버를 시작하는 대신 기존 서버에 연결합니다. |
| `plantumlLocalServerHost` | `127.0.0.1` | 자동 시작이 **꺼짐**일 때 연결할 호스트(예: LAN의 다른 위치에 있는 picoweb 서버). 자동 시작이 켜짐일 때는 무시됩니다 — 관리되는 서버는 항상 `127.0.0.1`에 바인딩됩니다. |
| `plantumlLocalServerPort` | `0` | 자동 시작이 **켜짐**일 때: 시작할 포트(`0` = 사용 가능한 포트를 자동 할당). 자동 시작이 **꺼짐**일 때: 연결할 포트. |

참고:

- 자동 시작이 **켜짐**이고 고정 포트를 사용할 때, 해당 포트에서 정상 동작 중인 PlantUML 서버가 이미 있으면 확장 프로그램은 새로 시작하지 않고 그 서버를 재사용합니다 — 따라서 남아 있는 프로세스나 직접 시작한 서버와 더 이상 충돌하지 않습니다.
- 자동 시작이 **꺼짐**이면 확장 프로그램은 서버를 시작하지 않으며(로컬 Java도 필요 없음) 설정한 호스트/포트에 연결만 합니다.
