<p align="center">
  <a href="https://yss-tazawa.github.io/plantuml-markdown-preview/">English</a> | <a href="https://yss-tazawa.github.io/plantuml-markdown-preview/#/zh-cn/">简体中文</a> | <a href="https://yss-tazawa.github.io/plantuml-markdown-preview/#/ja/">日本語</a> | <a href="https://yss-tazawa.github.io/plantuml-markdown-preview/#/es/">Español</a> | <a href="https://yss-tazawa.github.io/plantuml-markdown-preview/#/pt-br/">Português</a> | <strong>한국어</strong> | <a href="https://yss-tazawa.github.io/plantuml-markdown-preview/#/zh-tw/">繁體中文</a>
</p>
...
<p align="center">
  <img src="images/hero-screenshot.png" width="800" alt="GitHub Light 테마에서 시퀀스 다이어그램을 보여주는 에디터와 미리보기의 나란한 모습">
</p>
...
- **D2 배율** — `auto` (컨테이너에 맞춤) 또는 고정 비율 (50%–100%, 기본값 70%).
...
| **속도** | 즉각적 (상주 로컬 서버) | 느림 (매번 JVM 시작) | 네트워크 상태에 따라 다름 |

- **Fast 모드** (기본값) — `localhost`에서 상주 PlantUML 서버를 실행하며, 첫 다이어그램 렌더링 시점에 지연 시작됩니다(설정 가능 — [Start Mode](#fast-모드-서버-시작-모드) 참조). 편집할 때마다 발생하는 JVM 시작 비용을 제거하여 즉각적인 재렌더링이 가능합니다. 다이어그램은 사용자의 컴퓨터를 벗어나지 않습니다.
...
기본적으로 Fast 모드는 `127.0.0.1`에 바인딩된 PlantUML 서버를 직접 시작하고 관리합니다 — 별도의 설정이 필요 없습니다. `plantumlLocalServerStartMode`는 서버를 언제 시작할지(그리고 시작할지 여부)를 결정합니다:

| 값 | 효과 |
|---|---|
| `"lazy"` (기본값) | 첫 다이어그램 렌더링 시점에 관리되는 서버를 시작합니다. 다이어그램을 렌더링하지 않는 동안에는 JVM이 실행되지 않으며, 첫 렌더링에는 몇 초 정도 더 걸립니다. |
| `"on"` | 확장 프로그램이 활성화되는 즉시 관리되는 서버를 시작합니다. 첫 렌더링은 즉각적이지만, 시작 시점부터 JVM이 상주하는 대가가 따릅니다. |
| `"off"` | 서버를 전혀 시작하지 않습니다. 직접 실행하는 서버(예: `java -jar plantuml.jar -picoweb`)에 연결하며, LAN 내 다른 컴퓨터에서 실행 중인 서버도 지정할 수 있습니다: |

| 설정 | 기본값 | 효과 |
|---|---|---|
| `plantumlLocalServerHost` | `127.0.0.1` | Start Mode가 `"off"`일 때 연결할 호스트(예: LAN의 다른 위치에 있는 picoweb 서버). `"on"`/`"lazy"`에서는 무시됩니다 — 관리되는 서버는 항상 `127.0.0.1`에 바인딩됩니다. |
| `plantumlLocalServerPort` | `0` | Start Mode `"on"`/`"lazy"`: 시작할 포트(`0` = 사용 가능한 포트를 자동 할당). Start Mode `"off"`: 연결할 포트. |

참고:

- 관리되는 모드(`"on"`/`"lazy"`)에서 고정 포트를 사용할 때, 해당 포트에서 정상 동작 중인 PlantUML 서버가 이미 있으면 확장 프로그램은 새로 시작하지 않고 그 서버를 재사용합니다 — 따라서 남아 있는 프로세스나 직접 시작한 서버와 충돌하지 않습니다.
- Start Mode가 `"off"`이면 확장 프로그램은 서버를 시작하지 않으며(로컬 Java도 필요 없음) 설정한 호스트/포트에 연결만 합니다.
- 확장 프로그램은 자신이 시작한 서버만 중지합니다. 사용자가 직접 실행한 서버는 확장 프로그램에 의해 종료되지 않습니다.
- 이전의 `plantumlLocalServerAutoStart` 불리언 설정은 더 이상 사용되지 않지만(deprecated), Start Mode를 명시적으로 설정하지 않은 경우에는 여전히 적용됩니다(`true` → `"on"`, `false` → `"off"`). Start Mode를 명시적으로 설정하면 항상 그 값이 우선합니다.

#### Fast 모드: JVM 메모리(힙) 설정

관리되는 서버의 메모리 사용량은 힙 프리셋(`plantumlLocalServerJvmHeapPreset`)으로 제한됩니다:

| 프리셋 | JVM 힙 플래그 | 적합한 경우 |
|---|---|---|
| `small` | `-Xms16m -Xmx256m` | 메모리 사용량 최소화; 작은 다이어그램 전용. |
| `medium` (기본값) | `-Xms16m -Xmx512m` | 균형 잡힌 설정; 대부분의 다이어그램을 무리 없이 처리합니다. |
| `large` | `-Xms64m -Xmx1024m` | 매우 큰 다이어그램용; PlantUML FAQ 권장값과 일치합니다. |
| `unlimited` | _(없음)_ | 플래그를 전혀 지정하지 않습니다 — JVM이 알아서 결정합니다(최대 힙은 기본적으로 물리 메모리의 1/4). 0.7.10 이전 동작으로 대체됩니다. |
| `custom` | 사용자 지정 값 | `plantumlLocalServerJvmInitialHeapMb`(8–32768, 기본값 16)와 `plantumlLocalServerJvmMaxHeapMb`(64–32768, 기본값 512)를 읽습니다. |

`unlimited`를 제외한 모든 프리셋은 서버를 가볍게 유지하기 위해 `-XX:+UseSerialGC`, `-XX:MaxMetaspaceSize=128m`, `-XX:ReservedCodeCacheSize=64m`도 함께 고정합니다. 이 설정은 확장 프로그램이 직접 시작한 서버(Fast 모드, Start Mode `"on"`/`"lazy"`)에만 적용됩니다.
...
- **픽셀 너비 프리셋** (640–1440px, 가운데 정렬)과 뷰포트 너비에 맞춰 확장되는 **반응형** 모드 (왼쪽 정렬)
...
- 세로 및 가로 방향 지원
- 텍스트와 다이어그램 균일 배율 조정 (`pdfScale` 설정, 기본값 0.625)
...
> **참고:** `plantumlIncludePath`(및 워크스페이스 루트 기준 해석)는 확장 프로그램 자신이 PlantUML 프로세스를 시작한 경우에만 적용됩니다 — Secure 모드에서는 항상 적용되지만, Fast 모드에서는 확장 프로그램이 자체 로컬 서버를 시작한 경우에만 적용됩니다. Fast 모드가 이미 실행 중인 서버에 연결하는 경우(Start Mode `"off"`, 또는 고정 포트에서 기존 서버를 재사용하는 경우)에는 대신 그 서버 자체의 작업 디렉토리를 기준으로 include가 해석됩니다. Fast 모드에서 이 설정이 확실히 적용되도록 하려면 포트를 `0`(자동)으로 설정하십시오.
...
**Fast 모드** (기본값): 즉각적인 재렌더링을 위해 상주 로컬 PlantUML 서버를 실행하며, 첫 다이어그램 렌더링 시점에 시작됩니다. Java 11+이 필요합니다.
...
- **컨텍스트 메뉴:** `.md` 파일 우클릭 → **PlantUML Markdown Preview** → **Export as HTML** → 너비 옵션 선택
- **미리보기 패널:** 미리보기 패널 내부 우클릭 → **Export as HTML** 또는 **Export as HTML & Open**

서브메뉴에서 다음 너비 옵션을 선택할 수 있습니다:

| 옵션 | 설명 |
| ------ | ----------- |
| Current Settings | 현재 설정으로 내보내기 (반응형 CSS 없음) |
| 640px – 1440px | 픽셀 단위 고정 본문 너비, 가운데 정렬 |
| Responsive (Left-Aligned) | 뷰포트 너비의 본문, 왼쪽 정렬; 다이어그램은 넘칠 때만 축소됨 |
| 640px – 1440px (Left-Aligned) | 고정 너비, 왼쪽 정렬 |

HTML 파일은 소스 `.md` 파일과 같은 위치에 저장됩니다.

### PDF로 내보내기

- **컨텍스트 메뉴:** `.md` 파일 우클릭 → **PlantUML Markdown Preview** → **Export as PDF** → **세로** (Portrait) 또는 **가로** (Landscape)
- **미리보기 패널:** 미리보기 패널 내부 우클릭 → **Export as PDF** 또는 **Export as PDF & Open** → **세로** (Portrait) 또는 **가로** (Landscape)

PDF 파일은 소스 `.md` 파일과 같은 위치에 저장됩니다. Chrome, Edge 또는 Chromium이 필요합니다. `pdfScale` 설정 (기본값 0.625)으로 텍스트와 다이어그램의 전체 배율을 조정할 수 있습니다.
...
| `d2Scale` | `"70%"` | D2 다이어그램 배율. `"auto"`는 컨테이너 너비에 맞게 조정합니다. 백분율(50%–100%)은 원래 크기의 해당 비율로 렌더링합니다. |
...
| `pdfScale` | `0.625` | PDF 내보내기 배율. 텍스트와 다이어그램을 균일하게 조절. 범위: 0.1–2.0. |
...
| `plantumlLocalServerStartMode` | `"lazy"` | 로컬 PlantUML 서버를 시작하는 시점(Fast 모드 전용): `"on"`(활성화 시), `"lazy"`(첫 다이어그램 렌더링 시), `"off"`(시작하지 않음 — 기존 서버에 연결). [Start Mode](#fast-모드-서버-시작-모드) 참조. |
| `plantumlLocalServerHost` | `"127.0.0.1"` | Start Mode가 `"off"`일 때 연결할 호스트(Fast 모드 전용). `"on"`/`"lazy"`에서는 무시됩니다. |
| `plantumlLocalServerPort` | `0` | 로컬 PlantUML 서버의 포트 (Fast 모드 전용). `0` = 사용 가능한 포트를 자동으로 할당합니다. Start Mode가 `"off"`이면 연결할 포트입니다. |
| `plantumlLocalServerJvmHeapPreset` | `"medium"` | 관리되는 로컬 서버의 JVM 힙 프리셋: `"small"`, `"medium"`, `"large"`, `"unlimited"`, `"custom"` 중 하나. [JVM 메모리 설정](#fast-모드-jvm-메모리힙-설정) 참조. |
| `plantumlLocalServerJvmInitialHeapMb` | `16` | `"custom"` 프리셋의 초기 힙(`-Xms`, MB). 다른 프리셋에서는 무시됩니다. 범위: 8–32768. |
| `plantumlLocalServerJvmMaxHeapMb` | `512` | `"custom"` 프리셋의 최대 힙(`-Xmx`, MB). 다른 프리셋에서는 무시됩니다. 범위: 64–32768. |
...
**Fast 모드**(`mode: "fast"`)로 전환하십시오. localhost에서 상주 PlantUML 서버를 시작하므로 편집할 때마다 발생하는 JVM 시작 비용이 없어 즉각적인 재렌더링이 가능합니다.
...
- Fast 모드에서는 include 기준 디렉토리가 확장 프로그램이 자체 로컬 서버를 시작한 경우에만 적용됩니다. 이미 실행 중인 서버에 연결하는 경우(Start Mode `"off"`, 또는 고정 포트에서 기존 서버를 재사용하는 경우)에는 대신 그 서버의 작업 디렉토리를 기준으로 include가 해석됩니다 — 확장 프로그램이 항상 자체 서버를 시작하도록 하려면 포트를 `0`(자동)으로 설정하십시오.
- 포함된 파일을 저장하면 미리보기가 자동으로 새로고침됩니다. **Reload** 버튼(↻)을 클릭하여 수동으로 강제 새로고침할 수도 있습니다.
